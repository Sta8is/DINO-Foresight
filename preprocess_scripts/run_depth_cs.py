...
if __name__ == '__main__':
    ...
    for k, filename in tqdm(enumerate(filenames)):
        # out_dir = filename.replace('/leftImg8bit/','/leftImg8bit_depth/')
        out_dir = filename.replace('leftImg8bit','leftImg8bit_depthv2')
        ...
        if args.pred_only:
            ...
            # cv2.imwrite(out_dir, depth)
            depth_pil = Image.fromarray(depth).convert("L")
            depth_pil.save(out_dir)
        else:
